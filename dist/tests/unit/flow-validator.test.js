...
const FlowValidator_1 = require("../../core/validation/FlowValidator");
...
describe("FlowValidator", () => {
    describe("Schema Validation", () => {
        test("should validate a valid basic flow", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(validFlow);
        ...
        test("should reject flow missing required fields", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(invalidFlow);
        ...
        test("should validate variable types", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(flowWithTypes);
            expect(result.isValid).toBe(true);
        });
        test("should reject invalid variable types", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(invalidTypeFlow);
            expect(result.isValid).toBe(false);
        });
    });
    describe("Node Validation", () => {
        test("should validate LLM node structure", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(llmFlow);
            expect(result.isValid).toBe(true);
        });
        test("should validate CONDITION node structure", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(conditionFlow);
            expect(result.isValid).toBe(true);
        });
        test("should validate FOR_EACH node structure", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(forEachFlow);
            expect(result.isValid).toBe(true);
        });
    });
    describe("Variable Reference Validation", () => {
        test("should handle forward references correctly", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(forwardReferenceFlow);
            ...
            expect((_a = result.dependencyGraph) === null || _a === void 0 ? void 0 : _a.executionOrder).toEqual(["node_b", "node_a"]);
        });
        test("should detect circular dependencies", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(circularFlow);
            expect(result.isValid).toBe(false);
            expect(result.errors.some((err) => err.code.includes("CIRCULAR_DEPENDENCY"))).toBe(true);
        });
        test("should validate variable interpolation syntax", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(interpolationFlow);
            expect(result.isValid).toBe(true);
        });
    });
    describe("Unique Identifier Validation", () => {
        test("should reject duplicate node IDs", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(duplicateIdFlow);
            expect(result.isValid).toBe(false);
            console.log("errors ", result.errors);
            expect(result.errors.some((err) => err.code.includes("DUPLICATE_NODE_ID"))).toBe(true);
        });
        test("should reject duplicate variable IDs", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(duplicateVarFlow);
            expect(result.isValid).toBe(false);
            expect(result.errors.some((err) => err.code.includes("DUPLICATE_VARIABLE_ID"))).toBe(true);
        });
    });
    describe("Protocol Compliance", () => {
        test("should validate semantic versioning format", () => {
            ...
            versions.forEach(({ version, valid }) => {
                ...
                const result = FlowValidator_1.FlowValidator.validateFlow(flow);
        ...
        test("should validate input/output variable references", () => {
            ...
            const result = FlowValidator_1.FlowValidator.validateFlow(invalidIOFlow);
            expect(result.isValid).toBe(false);
        });
    });
});
