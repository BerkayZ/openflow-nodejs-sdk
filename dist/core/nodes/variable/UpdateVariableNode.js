...
exports.UpdateVariableNodeExecutor = void 0;
const BaseNode_1 = require("../base/BaseNode");
class UpdateVariableNodeExecutor extends BaseNode_1.BaseNode {
    /**
    ...
    execute(node, context) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const resolvedConfig = this.resolveConfigVariables(updateNode.config, context.registry);
            ...
            const resolvedValue = typeof updateNode.value === "string"
            ...
            const newValue = this.performUpdateOperation(type, currentValue, resolvedValue, Object.assign({ join_str }, resolvedConfig));
            // Update the variable in registry
            context.registry.setVariable(variable_id, newValue);
            this.log(context, "info", `Updated variable ${variable_id} from ${currentValue} to ${newValue}`);
     ...
     */
    validateTargetVariable(variableId, context) {
        if (!context.registry.hasVariable(variableId)) {
            this.log(context, "warn", `Target variable '${variableId}' does not exist, will be created`);
        }
    }
    /**
     * Perform the update operation based on type
     */
    performUpdateOperation(type, currentValue, resolvedValue, config) {
        switch (type) {
            case "join": {
                const separator = config.join_str !== undefined ? config.join_str : "";
                const shouldStringify = config.stringify_output !== undefined
                    ? config.stringify_output
                    : true;
                if (currentValue !== undefined) {
                    const currentStr = typeof currentValue === "object" && shouldStringify
                        ? JSON.stringify(currentValue)
                        : String(currentValue);
                    const resolvedStr = typeof resolvedValue === "object" && shouldStringify
            ...
            case "pick": {
                ...
                const shouldStringify = config.stringify_output !== undefined
                    ...
                    : false;
                if (Array.isArray(resolvedValue)) {
                    const result = resolvedValue.map((item) => {
                        const picked = {};
            ...
            case "omit": {
                ...
                if (Array.isArray(resolvedValue)) {
                    return resolvedValue.map((item) => {
                        if (!item || typeof item !== "object")
                            return item;
                        const omitted = Object.assign({}, item);
                        for (const field of config.fields) {
                    ...
                    const omitted = Object.assign({}, resolvedValue);
                    for (const field of config.fields) {
                        // Handle nested field paths for omit
                        if (field.includes(".")) {
                            this.deleteNestedValue(omitted, field);
            ...
            case "filter": {
                ...
                return resolvedValue.filter((item) => {
...
exports.UpdateVariableNodeExecutor = UpdateVariableNodeExecutor;
