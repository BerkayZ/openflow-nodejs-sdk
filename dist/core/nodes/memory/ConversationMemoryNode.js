...
exports.ConversationMemoryNode = void 0;
const BaseNode_1 = require("../base/BaseNode");
class ConversationMemoryNode extends BaseNode_1.BaseNode {
    execute(node, context) {
        return __awaiter(this, void 0, void 0, function* () {
            const memoryNode = node;
            const config = memoryNode.config;
    ...
    appendMessage(messages, input, config, context) {
        ...
        context.logger.debug(`Appended message to conversation. Total messages: ${messages.length}`);
        return {
            message_count: messages.length,
            last_message: newMessage,
        };
    }
    loadMessages(input, config, context) {
        // Resolve input variables
        const resolvedInput = this.resolveObjectVariables(input, context.registry);
    ...
    sliceMessages(messages, config, context) {
        ...
        context.logger.debug(`Sliced messages from ${start} to ${end}. New count: ${slicedMessages.length}`);
        return {
            message_count: slicedMessages.length,
            sliced: true,
        };
    }
    serializeMessages(messages, context) {
    ...
    deserializeMessages(input, config, context) {
        // Resolve input variables
        const resolvedInput = this.resolveObjectVariables(input, context.registry);
...
exports.ConversationMemoryNode = ConversationMemoryNode;
