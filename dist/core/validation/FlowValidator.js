...
class FlowValidator {
     ...
     */
    static validateFlow(jsonData, availableProviders) {
        const errors = [];
        const warnings = [];
        try {
            ...
            errors.push({
                ...
                code: types_1.ValidationErrorCode.INVALID_FORMAT,
            });
            return { isValid: false, errors, warnings };
        }
    }
     ...
     */
    static parseAndValidateStructure(jsonData) {
        const errors = [];
        const warnings = [];
        ...
        if (!jsonData || typeof jsonData !== "object") {
            errors.push({
                ...
                code: types_1.ValidationErrorCode.INVALID_TYPE,
            });
            return { isValid: false, errors, warnings };
        }
        // Required fields
        ...
        for (const field of requiredFields) {
            if (!jsonData[field] || typeof jsonData[field] !== "string") {
                errors.push({
                    ...
                    code: types_1.ValidationErrorCode.MISSING_REQUIRED_FIELD,
                });
            }
        }
        // Validate semantic versioning format for version field
        if (jsonData.version && typeof jsonData.version === "string") {
            const semverValidation = this.validateSemanticVersioning(jsonData.version);
            if (!semverValidation.isValid) {
                errors.push({
                    path: "version",
                    message: semverValidation.message ||
     ...
     */
    static validateDependenciesAndVariables(flow) {
        const errors = [];
        const warnings = [];
        // Extract variable references
        const variableReferences = this.extractVariableReferences(flow);
        // Validate variable references
        const variableValidation = this.validateVariableReferences(flow, variableReferences);
        errors.push(...variableValidation.errors);
        warnings.push(...variableValidation.warnings);
        // Build dependency graph
        const dependencyResult = this.buildDependencyGraph(flow, variableReferences);
        errors.push(...dependencyResult.errors);
        warnings.push(...dependencyResult.warnings);
        if (dependencyResult.graph) {
            // Validate execution order
            const executionValidation = this.validateExecutionOrder(flow, dependencyResult.graph);
     ...
     */
    static validateNodesStructure(nodes) {
        const errors = [];
        const warnings = [];
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (!node.id || typeof node.id !== "string") {
                ...
                if (nodeIds.has(node.id)) {
                    errors.push({
                        ...
                        code: types_1.ValidationErrorCode.DUPLICATE_NODE_ID,
                    });
                }
                else {
                    nodeIds.add(node.id);
            ...
            if (!node.type || !validTypes.includes(node.type)) {
                errors.push({
                    ...
                    code: types_1.ValidationErrorCode.INVALID_NODE_TYPE,
                });
            }
            ...
            const nodeSpecificValidation = this.validateNodeSpecificStructure(node, nodePath);
     ...
     */
    static validateNodeSpecificStructure(node, nodePath) {
        ...
        switch (node.type) {
            case "LLM":
                // Validate config
                if (!((_a = node.config) === null || _a === void 0 ? void 0 : _a.provider) || !((_b = node.config) === null || _b === void 0 ? void 0 : _b.model)) {
                    errors.push({
            ...
            case "DOCUMENT_SPLITTER":
                ...
                break;
            case "TEXT_EMBEDDING":
                if (!((_c = node.config) === null || _c === void 0 ? void 0 : _c.provider) || !node.input) {
                    errors.push({
                        path: `${nodePath}`,
            ...
            case "VECTOR_UPDATE":
            case "VECTOR_DELETE":
                if (!((_d = node.config) === null || _d === void 0 ? void 0 : _d.provider) || !((_e = node.config) === null || _e === void 0 ? void 0 : _e.index_name) || !node.input) {
                    errors.push({
                        path: `${nodePath}`,
                ...
                break;
            case "FOR_EACH":
                if (!((_f = node.config) === null || _f === void 0 ? void 0 : _f.each_key) ||
                    !((_g = node.input) === null || _g === void 0 ? void 0 : _g.items) ||
            ...
            case "UPDATE_VARIABLE":
                if (!((_h = node.config) === null || _h === void 0 ? void 0 : _h.variable_id) || !node.value) {
                    errors.push({
                        path: `${nodePath}`,
                ...
                break;
            case "CONDITION":
                if (!((_j = node.input) === null || _j === void 0 ? void 0 : _j.switch_value) || !node.branches) {
                    errors.push({
                        path: `${nodePath}`,
     ...
     */
    static validateVariablesStructure(variables) {
        const errors = [];
        const warnings = [];
        ...
        for (let i = 0; i < variables.length; i++) {
            ...
            if (!(variable === null || variable === void 0 ? void 0 : variable.id) || typeof variable.id !== "string") {
                errors.push({
                ...
                errors.push({
                    ...
                    code: types_1.ValidationErrorCode.DUPLICATE_VARIABLE_ID,
                });
            }
            else {
     ...
     * Validate input/output variable references
     */
    static validateInputOutputReferences(flow) {
        const errors = [];
        const warnings = [];
        ...
        for (let i = 0; i < flow.input.length; i++) {
            ...
            if (!variableIds.has(inputVarId)) {
                errors.push({
                    ...
                    code: types_1.ValidationErrorCode.INVALID_VARIABLE_REFERENCE,
                });
            }
        }
     ...
     */
    static extractVariableReferences(flow) {
        const references = [];
        const extractFromText = (text, scopeContext) => {
            const regex = /\{\{([^}]+)\}\}/g;
            let match;
        ...
        const extractFromNodes = (nodes, scopeContext) => {
            var _a;
            for (const node of nodes) {
                if (node.type === "FOR_EACH") {
                    // Create scope context for FOR_EACH nodes
                    const forEachScopeNodes = new Set();
                    const scopeKeys = new Set();
                    // Add the each_key to scope keys
                    if ((_a = node.config) === null || _a === void 0 ? void 0 : _a.each_key) {
                    ...
                    const collectScopeNodes = (eachNodes) => {
                    ...
                    const newScopeContext = {
                        ...
                        scopeKeys: new Set([
                            ...((scopeContext === null || scopeContext === void 0 ? void 0 : scopeContext.scopeKeys) || []),
                            ...scopeKeys,
                        ]),
                    };
                    // Extract from the FOR_EACH node itself (input, config, etc.)
                    const nodeWithoutEachNodes = Object.assign({}, node);
                    delete nodeWithoutEachNodes.each_nodes;
     ...
     */
    static validateVariableReferences(flow, references) {
        var _a, _b, _c, _d;
        const errors = [];
        const warnings = [];
        ...
        for (const reference of references) {
            ...
            if ((_b = (_a = reference.scopeContext) === null || _a === void 0 ? void 0 : _a.scopeKeys) === null || _b === void 0 ? void 0 : _b.has(reference.nodeId)) {
            ...
            if ((_d = (_c = reference.scopeContext) === null || _c === void 0 ? void 0 : _c.forEachNodes) === null || _d === void 0 ? void 0 : _d.has(reference.nodeId)) {
                isValidNodeReference = true;
            }
     ...
     * Build dependency graph
     */
    static buildDependencyGraph(flow, references) {
        const errors = [];
        const warnings = [];
        const graph = {
            nodes: new Map(),
        ...
        try {
            ...
            errors.push({
                ...
                code: types_1.ValidationErrorCode.CIRCULAR_DEPENDENCY,
            });
            return { isValid: false, errors, warnings };
        }
     ...
     */
    static extractNodeReferences(node) {
        const references = [];
            ...
            case "LLM":
                ...
                if (node.messages) {
                    for (const message of node.messages) {
                        if (message.text)
                            extractFromText(message.text);
                        if (message.image_url)
                            extractFromText(message.image_url);
                        if (message.image_path)
                            extractFromText(message.image_path);
                    }
                }
                break;
     ...
     * Validate execution order
     */
    static validateExecutionOrder(flow, graph) {
        const errors = [];
        const warnings = [];
        ...
        if (orderSet.size !== nodeSet.size) {
            errors.push({
                ...
                code: types_1.ValidationErrorCode.MISSING_DEPENDENCY,
            });
        }
        return { isValid: errors.length === 0, errors, warnings };
    }
     ...
     * Validate providers
     */
    static validateProviders(flow, availableProviders) {
        var _a, _b;
        const errors = [];
        const warnings = [];
        for (const node of flow.nodes) {
            ...
            switch (node.type) {
                case "LLM":
                case "TEXT_EMBEDDING":
                    requiredProvider = (_a = node.config) === null || _a === void 0 ? void 0 : _a.provider;
                    break;
                case "VECTOR_INSERT":
                case "VECTOR_SEARCH":
                case "VECTOR_UPDATE":
                case "VECTOR_DELETE":
                    requiredProvider = (_b = node.config) === null || _b === void 0 ? void 0 : _b.provider;
                    break;
            }
            if (requiredProvider && !availableProviders.has(requiredProvider)) {
                errors.push({
                    ...
                    code: types_1.ValidationErrorCode.MISSING_PROVIDER_CONFIG,
                });
            }
        }
        return { isValid: errors.length === 0, errors, warnings };
     ...
     */
    static validateSemanticVersioning(version) {
...
FlowValidator.logger = new Logger_1.Logger();
