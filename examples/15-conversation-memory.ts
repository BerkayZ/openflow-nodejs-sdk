...
import { Flow, LLMNode, ConversationMemoryNode, NodeType } from '../src/core/types';
...
async function demonstrateMemoryOperations() {
    ...
    console.log('History:', JSON.stringify(historyResult.outputs.memory_op?.history, null, 2));

    // Get summary
    console.log('\nGenerating conversation summary...');
    memoryOpsFlow.variables.memory_action = 'summary';
    const summaryResult = await executor.executeFlow(memoryOpsFlow);
    console.log('Summary:', JSON.stringify(summaryResult.outputs.memory_op?.summary, null, 2));
